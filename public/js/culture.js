...
function main() {
  ...
  $('#tagUsersModal').on('show.bs.modal', function (event) {
    ...
    if (itemId = $(button).data('id')) {
      ...
      var url = baseUrl + '/user/ajax/getCultCommentTaggedUsers/' + itemId;
      var request = $.ajax({
        method: 'get',
        url: url
      });
...
function loadReplies(selected) {
  ...
  var url = baseUrl + "/user/ajax/getCultReplies/" + parentId;
  var request = $.ajax({
    method: 'get',
    url: url,
    data: {
...
function loadMoreComments(selected) {
  ...
  var url = baseUrl + "/user/ajax/getCultComments/" + itemId;
  var request = $.ajax({
    method: 'get',
    url: url,
    data: {
  ...
  if (data[0].value.trim() != "") {
    ...
    request.done(function (response) {
      if (response.status === 'success') {
        ...
        $('[data-tool="tooltip"]').tooltip();
        $('.commentDelete').off('click');
        var commentAmount = $('#post' + itemId).find('.postCommentsCount').html().trim();
