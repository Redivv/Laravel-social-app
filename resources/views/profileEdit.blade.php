...
<div class="container">
    <form id="profileEditForm" action="{{route('ProfileUpdate')}}" class="row text-center" method="post"
        ...
        <fieldset class="form-group col-12 row profilePicture">
            ...
            <label for="profilePictureInput" class="col-12 col-form-label">
                <img src="{{asset('img/profile-pictures/'.$user->picture)}}"
                    alt="{{__("profile.photo", ['user' => $user->name])}}">
        ...
        <fieldset class="form-group col-12 row profileCity">
            ...
            <input type="text" class="form-control" name="profileCity" id="profileCityInput"
                value="@if($user->city_id){{$user->city->name}}@endif">
        </fieldset>
        ...
        <fieldset class="form-group col-12 row profileRelationship">
            ...
            <div class="form-check form-check-inline col row">
                <input class="form-check-input col-12" type="radio" name="profileRelationship"
                    id="profileRelationshipInput1" value="0" @if($user->relationship_status === 0) checked @endif >
                <label class="form-check-label col-12"
                    for="profileRelationshipInput1">{{__('profile.status_free')}}</label>
            </div>
            <div class="form-check form-check-inline col row">
                <input class="form-check-input col-12" type="radio" name="profileRelationship"
                    id="profileRelationshipInput2" value="1" @if($user->relationship_status === 1 ||
                $user->relationship_status === 4) checked @endif>
                <label class="form-check-label col-12"
                    for="profileRelationshipInput2">{{__('profile.status_taken')}}</label>
            </div>
            <div class="form-check form-check-inline col row">
                <input class="form-check-input col-12" type="radio" name="profileRelationship"
                    id="profileRelationshipInput3" value="2" @if($user->relationship_status === null) checked @endif>
                <label class="form-check-label col-12"
                    for="profileRelationshipInput3">{{__('profile.status_hidden')}}</label>
            </div>


            <span class="col-12 tagPartner"><i class="fas fa-user-tag" data-toggle="modal"
                    data-target="#tagPartnerModal" data-tool="tooltip" title="{{__('profile.tagPartner')}}"
            ...
            <div id="userPartner" class="col-12">
                ...
                <div class="userFriend col">
                    <div class="userFriendContainer">
                        ...
                        <img src="{{asset('img/profile-pictures/'.$user->partner->picture)}}"
                            alt="{{__('profile.photo', ['user' => $user->partner->name])}}">
        ...
        <fieldset class="form-group col-12 row profileTags">
            ...
            <input type="text" class="form-control" name="profileTags[]" id="profileTagsInput"
                placeholder="Wpisz zainteresowanie i zatwierdź enterem">
            <output id="profileTagsOut" class="container row">
                @foreach ($tags as $tag)
                <div class="col-2 userTag" data-tool="tooltip" title="{{__('activityWall.deleteTags')}}"
                ...
                if (response.status === 'success') {
                    ...
                    $('#tagPartnerButton').one('click', function () {
                        ...
                        $('#deletePartner').on('click', function () {
                            if (confirm(deletePartnerMsgs)) {
                                html =
                                '<input type="hidden" name="deletePartner" value="true">';
                                $('#deletePartner').parents('.userFriend').replaceWith(html);
                            }
                        });
                    });
